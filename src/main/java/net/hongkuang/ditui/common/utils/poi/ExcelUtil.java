package net.hongkuang.ditui.common.utils.poi;
...
public class ExcelUtil<T> {
     ...
     * @return 转换后集合
     */
    public List<T> importExcelIncludeImage(String sheetName, InputStream input, String extention) throws Exception {
        List<T> list = new ArrayList<T>();
        // 延迟解析
        ...
        } else if (extention.endsWith(".xlsx")) {
        ...
        if (rows > 0) {
            ...
            for (int i = 1; i < rows; i++) {
                ...
                for (int j = 0; j < cellNum; j++) {
                    Cell cell = row.getCell(j);
                    PictureData pictureData = maplist.get(i + "-" + j);
                    if (cell == null && pictureData == null) {
                        continue;
                    }
                    String c = null;
                    if (pictureData != null) {
                        byte[] fileBytes = pictureData.getData();
                        String fileExtension = pictureData.suggestFileExtension();
                        String fileName = RandomUtil.genString() + "." + fileExtension;
                        FileUtils.writeBytes(RuoYiConfig.getUploadFolderPath() + File.separator + fileName, fileBytes);
                        c = fileName;
                    } else {
                        // 先设置Cell的类型，然后就可以把纯数字作为String类型读进来了
                        row.getCell(j).setCellType(CellType.STRING);
