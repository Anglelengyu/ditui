 ...
 */
@Controller
@RequestMapping("/busi/groundEmployee")
public class GroundEmployeeController extends BaseController {

    private String prefix = "busi/groundEmployee";
    ...
    @RequiresPermissions("busi:groundEmployee:view")
    @GetMapping()
     ...
     * 查询员工列表
     */
    @RequiresPermissions("busi:groundEmployee:list")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(SearchEmployee employee) {
        startPage();
        employee.setEmployeeType(EmployeeTypeEnum.GROUND.getCode());
        employee.setEmployeeKey(EmployeeKeyEnum.GROUND.getCode());
        List<Employee> list = employeeService.selectEmployeeList(employee);
        return getDataTable(list);
    }

    @Log(title = "员工管理", businessType = BusinessType.EXPORT)
    @RequiresPermissions("busi:groundEmployee:export")
    @PostMapping("/export")
    @ResponseBody
    ...
    public String add(@PathVariable("teamId") Long teamId, ModelMap mmap) {
        ...
        roleService.selectRoleAll().forEach(role -> {
            if (role.getRoleKey().equals(EmployeeKeyEnum.GROUND.getCode())) {
                mmap.put("role", role);
            }
        });
     ...
     * 新增保存员工
     */
    @RequiresPermissions("busi:groundEmployee:add")
     ...
     * 修改保存员工
     */
    @RequiresPermissions("busi:groundEmployee:edit")
    ...
    @RequiresPermissions("busi:groundEmployee:remove")
