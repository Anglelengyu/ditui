package net.hongkuang.ditui.project.system.menu.service.impl;
...
public class MenuServiceImpl implements IMenuService {
    ...
    public List<Menu> selectMenusByUser(User user) {
        ...
        if (user.isAdmin()) {
            ...
            menus = menuMapper.selectUserMenusByUserId(user.getUserId());
        }
     ...
     */
    @Override
    public List<Map<String, Object>> userRoleMenuTreeData(List<Role> roleList, Long userId) {
        List<Map<String, Object>> trees = new ArrayList<Map<String, Object>>();
        List<Menu> menuList = new ArrayList<>();
        for (Role role : roleList) {
            menuList.addAll(menuMapper.selectMenusByRoleId(role.getRoleId()));
        }
        List<Menu> menusList = new ArrayList<>();
        for (Menu menu : menuList) {
            if (menu.getMenuId() == 2152 || menu.getMenuId() == 2153) {
                menusList.add(menu);
            }
        }
        menuList.removeAll(menusList);
        if (StringUtils.isNotNull(roleList)) {
            List<String> roleMenuList = menuMapper.selectUserMenuTree(userId);
            trees = getTrees(menuList, true, roleMenuList, true);
        } else {
            trees = getTrees(menuList, false, null, true);
        }
