...
$(function () {
    ...
    // laydate time-input 时间控件绑定
    if ($(".time-picker-input").length > 0) {
        layui.use('laydate', function () {
            var laydate = layui.laydate;
            var times = $(".time-picker-input");
            for (var i = 0; i < times.length; i++) {
                var time = times[i];
                laydate.render({
                    elem: time,
    ...
    $("#bootstrap-table").on("check.bs.table uncheck.bs.table check-all.bs.table uncheck-all.bs.table", function () {
        ...
        $('#toolbar .btn-balance-scale').toggleClass('disabled', !ids.length);
        $('#toolbar .btn-hand-sale').toggleClass('disabled', !ids.length);

        $('#toolbar .btn-pass').toggleClass('disabled', !ids.length);
        $('#toolbar .btn-reject').toggleClass('disabled', !ids.length);
